...
function AttachmentCard({ attachment }: { attachment: AttachmentInfo }) {
  ...
  return (
    <div className={`p-4 border ${attachment.isSuspicious ? 'bg-accent-red/5 border-accent-red/30' : 'bg-bg-panel border-border-color'} flex flex-col md:flex-row items-start md:items-center justify-between gap-4 transition-colors hover:border-accent-blue/50`}>
      <div className="flex items-start space-x-4 w-full md:w-auto overflow-hidden">
        <div className={`p-3 shrink-0 ${attachment.isSuspicious ? 'bg-accent-red/10 text-accent-red' : 'bg-bg-dark text-text-muted'}`}>
          ...
          {attachment.isSuspicious && attachment.suspiciousReasons.length > 0 && (
            <div className="mt-3 flex flex-wrap gap-2">
              {attachment.suspiciousReasons.map((reason, i) => (
                <span key={i} className="px-2 py-1 bg-accent-red/10 text-accent-red text-xs border border-accent-red/20 flex items-center space-x-1">
      ...
      <button
        ...
        className="shrink-0 p-2 text-text-muted hover:text-accent-blue hover:bg-accent-blue/10 transition-colors border border-transparent hover:border-accent-blue/20"
