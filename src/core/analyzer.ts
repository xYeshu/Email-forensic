...
export function calculateRiskScore(email: AnalyzedEmail): { riskScore: number, threatLevel: 'Safe' | 'Suspicious' | 'Malicious' | 'Unknown' } {
  let score = 0;

  // Authentication failures
  if (email.authResults.spf === 'Fail') score += 30;
  if (email.authResults.dkim === 'Fail') score += 30;
  if (email.authResults.dmarc === 'Fail') score += 40;

  // Attachments
  const hasSuspiciousAttachment = email.attachments.some(a => a.isSuspicious);
  if (hasSuspiciousAttachment) score += 50;
  ...
  // Suspicious Keywords
  if (email.iocs.keywords.length > 3) score += 15;
  else if (email.iocs.keywords.length > 0) score += 5;
  ...
  return { riskScore: Math.min(score, 100), threatLevel };
}
